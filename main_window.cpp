...
main_window::main_window()
{
    ...
    if (dictFile.open(QIODevice::ReadOnly | QIODevice::Text))
    {
        ...
        while (!in.atEnd())
        {
            ...
            if (!line.isEmpty())
            {
                m_dictionary.insert(line.toStdString());
            }
        }
    }
    else
    {
        QMessageBox::warning(this, "Warning", "Could not load dictionary file data/words.txt");
    }

    // 创建拼写检查高亮器
    m_spellChecker = std::make_unique<SpellChecker>(editor->document(), m_dictionary);

    // 启用自定义右键菜单
    editor->setContextMenuPolicy(Qt::CustomContextMenu);
    connect(editor, &QTextEdit::customContextMenuRequested, this, &main_window::show_spelling_suggestions);
}
...
void main_window::setup_tools_menu()
{
    ...
    auto* check_spelling = tool_menu->addAction("Check Spelling...");
    connect(check_spelling, &QAction::triggered, this, &main_window::rehighlight_spelling);
}
...
void main_window::show_spelling_suggestions(const QPoint& pos)
{
    ...
    if (m_spellChecker->isMisspelled(word))
    {
        QMenu menu;
        ...
        else
        {
            for (const QString& sug : suggestions)
            {
                ...
                connect(action, &QAction::triggered, this, [this, cursor, sug]()
                {
...
void main_window::rehighlight_spelling()
{
    if (m_spellChecker)
    {
...
void main_window::setup_view_menu()
{
    ...
    zoom_in->setShortcut(QKeySequence("Ctrl++")); // 注意：Qt 接受 "Ctrl++"
    connect(zoom_in, &QAction::triggered, this, &main_window::zoom_in);

    auto* zoom_out = view_menu->addAction("Zoom Out");
    zoom_out->setShortcut(QKeySequence("Ctrl+-"));
    connect(zoom_out, &QAction::triggered, this, &main_window::zoom_out);

    auto* zoom_reset = view_menu->addAction("Reset Zoom");
    zoom_reset->setShortcut(QKeySequence("Ctrl+0"));
    connect(zoom_reset, &QAction::triggered, this, &main_window::zoom_reset);
}
...
void main_window::zoom_reset()
{
    ...
    font.setPointSize(12); // 直接设为 12 号字体
    editor->setFont(font);
